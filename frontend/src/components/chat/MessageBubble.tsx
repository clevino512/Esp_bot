...
function SourceCard({ source, onOpen }: { source: Source; onOpen: (s: Source) => void }) {
...
export function MessageBubble({ message, onFeedback }: MessageBubbleProps) {
  ...
  const [localFeedback,  setLocalFeedback]  = useState<'helpful' | 'not_helpful' | null>(
    message.feedback === 'helpful'     ? 'helpful'     :
    message.feedback === 'not_helpful' ? 'not_helpful' : null
  )
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={clsx(
            ...
            isUser ? 'bg-neutral-200 dark:bg-neutral-700' : 'bg-primary-600'
          )}
        >
          {isUser
      ...
      {/* Source detail modal — rendered outside the bubble */}
      <SourceDetailModal
