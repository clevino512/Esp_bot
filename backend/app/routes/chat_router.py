...
async def send_message(request: ChatRequest, db: DatabaseDep, current_user: OptionalUserDep = None):
    try:
        return await ChatService(db).process_message(
...
async def get_history(session_id: str, db: DatabaseDep):
    messages = await ChatService(db).get_history(session_id)
    if not messages:
...
async def submit_feedback(request: FeedbackRequest, db: DatabaseDep):
    success = await ChatService(db).submit_feedback(message_id=request.message_id, feedback_type=request.feedback.value)
    if not success:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Message not found")
    return BaseResponse(success=True, message="Feedback submitted successfully")
...
async def submit_sus_feedback(request: SUSFeedbackRequest):
    record = {
        ...
        "responses": request.responses, "score": request.score,
...
async def clear_conversation(session_id: str, db: DatabaseDep):
    success = await ChatService(db).clear_conversation(session_id)
    if not success:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Conversation not found")
